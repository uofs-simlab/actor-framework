...
// Option A: Serialised latency — cuStreamSynchronize after every async operation
// so each timing phase reflects true wall-clock GPU time (no pipelining).
// This matches the actor variants' per-operation blocking behaviour exactly and
// gives the fairest per-phase overhead comparison.
#include <cuda.h>
#include <iostream>
#include <vector>
#include <chrono>
...
// runMatrixMul: executes the kernel with full per-operation synchronisation and
...
double runMatrixMul(CUmodule module, CUfunction kernel, int N) {
    ...
    // ----------------------------------
    // H2D copy A  [OPTION A: sync after each async op]
    // ----------------------------------
    ...
    // ----------------------------------
    // H2D copy B  [OPTION A: sync after each async op]
    // ----------------------------------
    ...
    // ----------------------------------
    // Kernel launch + execution  [OPTION A: sync after launch]
    // ----------------------------------
    ...
    // ----------------------------------
    // D2H copy  [OPTION A: sync included, matches actor copy_to_host]
    // ----------------------------------
    auto t_d2h_start = clock::now();

    checkCU(cuMemcpyDtoHAsync(h_c.data(), d_c, bytes, stream), "cuMemcpyDtoHAsync");
    checkCU(cuStreamSynchronize(stream), "D2H sync");
    ...
    // ----------------------------------
    // Free device memory (outside timed window, same as original)
    // ----------------------------------
