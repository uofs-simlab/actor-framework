    ...
    ThreadSafeQueue<MatrixTask>& queue)
{
    ...
    while (queue.wait_pop(task)) {
        std::cout << "[SUPE] Starting solver for: " << task.path << std::endl;
        auto start_task =
            std::chrono::steady_clock::now();
        int iterations = 0;
        if (task.type == CGS_SOLVER) {
            iterations = solve_cg_async(
                cublas,
                cusparse,
                task,
                stream);
        ...
        auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(
                          end_task - start_task).count();
        std::cout << "[DONE] " << task.path << ": Converged in " << iterations 
                  << " iterations (" << duration << " ms)." << std::endl;
    }
...
int main(int argc, char** argv)
{
    ...
    std::vector<MatrixTask> matrix_pool =
        scan_for_matrices("/scratch/nqr159/matrix-collection/matrices/mixed", CGS_SOLVER);
