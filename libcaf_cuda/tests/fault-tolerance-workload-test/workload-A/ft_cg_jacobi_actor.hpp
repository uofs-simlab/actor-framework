...
template <class T = float>
behavior fault_tolerant_cg_jacobi_actor(stateful_actor<ft_cg_jacobi_state<T>>* self,
                                        std::string path,
  ...
  return {
    [=](start_atom) {
      ...
      // Jacobi setup: Extract D_inv
      int threads = 256;
      nd_range range((st.n + threads - 1) / threads, 1, 1, threads, 1, 1);
      st.d_ptr->launch_kernel_mem_ref(st.diag_prog->get_kernel(st.d_ptr->getId()), range,
      ...
      if constexpr (std::is_same_v<T, double>) st.d_ptr->dcopy(st.stream_id, st.n, st.b, st.r); else st.d_ptr->scopy(st.stream_id, st.n, st.b, st.r);
      if constexpr (std::is_same_v<T, double>) st.d_ptr->daxpy(st.stream_id, st.n, -1.0, st.w, st.r); else st.d_ptr->saxpy(st.stream_id, st.n, -1.0f, st.w, st.r);
      ...
      // Initial rho = r * z
      if constexpr (std::is_same_v<T, double>) st.d_ptr->ddot(st.stream_id, st.n, st.r, st.z, st.y_tmp); else st.d_ptr->sdot(st.stream_id, st.n, st.r, st.z, st.y_tmp);
    ...
    [=](cg_next_step_atom, int num_iters) {
      ...
      while (step_count < num_iters && st.iterations < st.max_iter && st.current_rho > threshold) {
        ...
        if (st.iterations > 1) {
          ...
          if constexpr (std::is_same_v<T, double>) st.d_ptr->dcopy(st.stream_id, st.n, st.z, st.p); else st.d_ptr->scopy(st.stream_id, st.n, st.z, st.p);
        }

        st.d_ptr->spmv_csr(st.stream_id, st.n, st.n, st.nnz, T{1}, st.A_rp, st.A_ci, st.A_val, st.p, T{0}, st.w, st.spmv_ws);
        if constexpr (std::is_same_v<T, double>) st.d_ptr->ddot(st.stream_id, st.n, st.p, st.w, st.y_tmp); else st.d_ptr->sdot(st.stream_id, st.n, st.p, st.w, st.y_tmp);
      ...
      if (code != CG_SUCCESS && !is_fatal) { st.strikes++; if (st.strikes < 3) code = CG_SUCCESS; }
      ...
      runner.copy_to_host_async(st.x, [=, supervisor = st.supervisor, path = st.path, self_h = actor_cast<actor>(self)](std::vector<T> sol) {
        ...
        if (converged || code != CG_SUCCESS) anon_mail(shutdown_atom_v).send(self_h);
      });
    },
